...
//   total_dollars: 1926107
  ...
  var name = prettyCase(candidate.candidate_name.split(",").reverse().join(" ").trim());
...
//   contributor_name: "CALIFORNIA DEMOCRATIC PARTY"
  ...
  para.push(
    choose(SALUTATIONS) + " my name is " + name + ", and I am running for " +
    prettyCase(candidate.office) + ". "
  );

  if (topContributors.length > 1) {
    para.push (
      choose(REQUESTS) + " you can be sure that my top priority " +
      "will be the best interests of the " + prettyCase(topContributors[0].contributor_name) +
      ", which " + choose(BIGGESTS) + ". Of course, I will also " + choose(FIGHTINGS) +
      " the " + prettyCase(topContributors[1].contributor_name) +
  ...
  if (topIndustries.length <= 1 && topContributors.length <= 1) {
    para.push(
      "Since I didn't raise any money, I am just running for what I believe in, " +
      "and to find out more, please see my campaign website and the " +
      prettyCase(candidate.party) + " party website for more information."
    );
  } else {
